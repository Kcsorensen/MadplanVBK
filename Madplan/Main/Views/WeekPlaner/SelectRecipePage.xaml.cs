...
namespace Main.Views.WeekPlaner
{
	...
	public partial class SelectRecipePage : ContentPage
	{
		...
		private IEnumerable<Recipe> sortedListOfRecipes;

		public SelectRecipePage(string day, string dishType)
		{
			InitializeComponent();
		...
		private async Task SaveSelected(string selectedMeal)
		{
            ...
            DataModel.Database.GetCollection<WeekSelections>().Update(weekSelections);
        }

		private async Task sortedListOfRecipesAsync(string mealType = null)
		{
            sortedListOfRecipes = new List<Recipe>();

            await Task.Run(() =>
            {
                if (mealType == null)
                {
                    sortedListOfRecipes = DataModel.Current.ListOfRecipes;
                }
                else
                {
                    if (mealType == MealType.FirstSnack || mealType == MealType.SecondSnack)
                    {
                        sortedListOfRecipes = DataModel.Current.ListOfRecipes.Where(a => a.Type == MealType.Snack);
                    }
                    else
                    {
                        sortedListOfRecipes = DataModel.Current.ListOfRecipes.Where(a => a.Type == mealType);
                    }
                }
            });

            BindingContext = sortedListOfRecipes;
